...
```
/
├── index.html    → 홈 허브. CV / 가치관 / 인생 로드맵 3개 버튼
├── cv.html       → CV 스타일 자기소개 (학력·경력·수상·자격증·기타활동)
├── values.html   → 가치관 54개 원칙 (10부 구성)
├── roadmap.html  → 비공개(비밀번호 게이트). 나이대별 계획·자산목표·사업전략·버킷리스트
└── README.md
```

각 서브페이지는 좌상단 "← 홈" 링크로 `index.html`에 연결됨.

### roadmap.html의 비밀번호 게이트에 대해

클라이언트 사이드 JS로 비밀번호를 체크하는 방식이라 **진짜 보안이 아님** — 정적 사이트라 콘텐츠 전체가 이미 파일 안에 담겨 배포되고, 페이지 소스나 개발자도구를 열면 비밀번호와 내용이 그대로 보임. 캐주얼한 접근을 막는 정도지, 진짜 민감한 수치(자산 목표 등)를 지키려면:
- 이 파일을 공개 GitHub 저장소/배포에서 아예 빼거나
- Vercel 등 호스팅 레벨의 실제 비밀번호 보호 기능(플랜에 따라 제공)을 쓸 것
...
- 번호(01~54, 01~05 등)는 실제로 순서/논리 구조가 있는 콘텐츠라서 붙인 것 (디자인 스킬 원칙상 순서 없는 콘텐츠에 번호 매기기는 지양). 새 섹션 추가 시도 이 기준 유지.
